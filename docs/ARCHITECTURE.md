...
6. `design_preset.render_preset()` が `designs/README.md` の書式で md を組み立てる。Palette の weight は役割内の出現比率（8% 未満は落とす）、向きのプロパティは落とす（水平の原木の `axis` だけ残す）。参考図は役割を 1 文字にした平面図（各列の最上段）と南からの立面図（幅 60・高さ 40 まで）。
